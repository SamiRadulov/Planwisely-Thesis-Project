...
@st.cache_resource
def load_thesis_model():
    with open('model.pkl', 'rb') as f:
        model = pickle.load(f)
    with open('model_features.json') as f:
        features = json.load(f)
    feat_df = pd.read_csv('features_final.csv')
    feat_df['week_start'] = pd.to_datetime(feat_df['week_start'])
    shap_df = pd.read_csv('shap_values_2021.csv')
    shap_df['week_start'] = pd.to_datetime(shap_df['week_start'])
    return model, features, feat_df, shap_df

THESIS_MODEL, THESIS_FEATURES, THESIS_FEAT_DF, THESIS_SHAP = load_thesis_model()
...
## ADD 2
def forecast_4_global(model, feature_cols, hist_df):
    delta    = hist_df['date'].iloc[-1] - hist_df['date'].iloc[-2]
    hist     = list(hist_df['sales'])
    hist_max = hist_df['sales'].max() * 3
    hist_p10 = max(0, hist_df['sales'].quantile(0.10) * 0.5)

    rows = []
    for i in range(4):
        nd = hist_df['date'].iloc[-1] + (i + 1) * delta
        r  = {
            'date':         nd,
            'year':         int(nd.isocalendar()[0]),
            'month':        nd.month,
            'week_of_year': int(nd.isocalendar()[1]),
            'week_cos':     np.cos(2 * np.pi * int(nd.isocalendar()[1]) / 52),
        }
        for lag in [1, 2, 4, 8]:
            r[f'lag_{lag}'] = hist[-lag] if len(hist) >= lag else 0.0
        for w in [4, 8]:
            h2 = hist[-w:]
        ...
        row_df = pd.DataFrame([[r.get(c, 0) for c in feature_cols]],
                               columns=feature_cols)
        pred   = float(np.clip(model.predict(row_df)[0], hist_p10, hist_max))
        hist.append(pred)
        r['forecast'] = pred
        rows.append(r)
...
def run_all(raw_bytes, sales_col, date_mode, date_col, year_col, month_col,
    ...
    model    = THESIS_MODEL
    features = THESIS_FEATURES
    feat_df  = THESIS_FEAT_DF.copy()
    shap_df  = THESIS_SHAP.copy()

    out  = {}
    pids = feat_df['product_id'].unique().tolist()

    for pid in pids:
        hist = feat_df[feat_df['product_id'] == pid].sort_values('week_start').copy()
        hist = hist.rename(columns={'week_start': 'date', 'sku_sold': 'sales'})
        if len(hist) < 20:
            continue

        fcast  = forecast_4_global(model, features, hist)

        y_true = hist['sales'].values
        y_pred = np.clip(model.predict(hist[features]), 0, None)
        r2_val = float(r2_score(y_true, y_pred))

        pid_shap       = shap_df[shap_df['product_id'] == pid].copy()
        shap_feat_cols = [c for c in pid_shap.columns
                          if c not in ['product_id', 'week_start']]
        shap_mean_abs    = pid_shap[shap_feat_cols].abs().mean()
        shap_mean_signed = pid_shap[shap_feat_cols].mean()
        pairs = sorted(
            [(f, shap_mean_abs[f], shap_mean_signed[f]) for f in shap_feat_cols],
            key=lambda x: -x[1]
        )
